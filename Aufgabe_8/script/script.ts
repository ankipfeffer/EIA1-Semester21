...
function playSample (sample: string) {
    var sounds: HTMLAudioElement = new Audio(sample);
    sounds.play ();
}
...
var min: number = 0;
var max: number = 8;
var remixnumber: number = Math.round(Math.random() * (max - min)) + min;
...
function deleteRemix () {
    beatplay = [];
    var remixnumber2: number = Math.round(Math.random() * (max - min)) + min;
